...
router.get('/classroomscreatedbyme', authTokenHandler, async (req, res) => {
    try {
        ...
        return responseFunction(res, 200, 'Classrooms fetch successfully', classrooms, true);
    } catch (err) {
        return responseFunction(res, 500, 'Internal server error', err, false);
    }
})
...
router.get('/getclassbyid/:classid', authTokenHandler, async (req, res) => {
    ...
    try{
        // TODO : add one more condition  -> if (classroom.owner == req.userid || classroom.students.includes(req.userid)){ popular posts }
        ...
        return responseFunction(res, 500, 'Intrenal server error', err, false);
    }
})
...
router.get('/stats', async(req, res) => { // newly added
    try{
        const classrooms = await Classroom.countDocuments();
        const posts = await Post.countDocuments();
        const resources = await Post.countDocuments();
...
router.get("/recent", async (req, res) => { //recent class
    try {
...
router.get("/dashboardstats", authTokenHandler, async (req, res) => {
    try {
        ...
        res.status(200).json({
            ok: true,
            data: {
                classrooms: myClassrooms.length,
                resources: 0,
                students: 0
            }
        });
